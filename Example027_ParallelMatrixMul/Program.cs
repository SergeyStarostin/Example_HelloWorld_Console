...
const int THREADS_NUMBER = 10; // Константа - количество потоков
...
void SerialMatrixMul(int[,] a, int[,] b)
{
    ...
    // Проверка на возможность умножения матриц, программа прерывается при неудовлетворительных условиях
...
void PrepareParallelMatrixMul(int[,] a, int[,] b) // Метод для подготовки параллельного вычисления
{
    if (a.GetLength(1) != b.GetLength(0)) throw new Exception("Нельзя умножить такие матрицы");
    int eachThreadCalc = N / THREADS_NUMBER; // переменная, в которой задаем количество вычислений на каждый поток
    Thread[] arr = new Thread[2];
    var threadsList = new List<Thread>(); // Коллекция
    for (int i = 0; i < THREADS_NUMBER; i++)
    {
        int startPos = i * eachThreadCalc; // позиция старта
        int endPos = (i + 1) * eachThreadCalc; // позиция завершения
